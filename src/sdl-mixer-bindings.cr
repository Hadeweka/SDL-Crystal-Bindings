...
lib LibSDL
  ...
  fun mix_open_audio = Mix_OpenAudio(devid : AudioDeviceID, spec : AudioSpec*) : Bool
  fun mix_pause_audio = Mix_PauseAudio(pause_on : LibC::Int) : Void
  fun mix_query_spec = Mix_QuerySpec(frequency : LibC::Int*, format : AudioFormat*, channels : LibC::Int*) : Bool
  fun mix_allocate_channels = Mix_AllocateChannels(numchans : LibC::Int) : LibC::Int
  fun mix_load_wav_io = Mix_LoadWAV_IO(src : IOStream*, closeio : Bool) : MixChunk*
  fun mix_load_wav = Mix_LoadWAV(file : LibC::Char*) : MixChunk*
  fun mix_load_mus = Mix_LoadMUS(file : LibC::Char*) : MixMusic*
  fun mix_load_mus_io = Mix_LoadMUS_IO(src : IOStream*, closeio : Bool) : MixMusic*
  fun mix_load_mustype_io = Mix_LoadMUSType_IO(src : IOStream*, type : MixMusicType, closeio : Bool) : MixMusic*
  ...
  fun mix_has_chunk_decoder = Mix_HasChunkDecoder(name : LibC::Char*) : Bool
  ...
  fun mix_has_music_decoder = Mix_HasMusicDecoder(name : LibC::Char*) : Bool
  ...
  fun mix_register_effect = Mix_RegisterEffect(chan : LibC::Int, f : MixEffectFuncT, d : MixEffectDoneT, arg : Void*) : Bool
  fun mix_unregister_effect = Mix_UnregisterEffect(channel : LibC::Int, f : MixEffectFuncT) : Bool
  fun mix_unregister_all_effects = Mix_UnregisterAllEffects(channel : LibC::Int) : Bool
  fun mix_set_panning = Mix_SetPanning(channel : LibC::Int, left : UInt8, right : UInt8) : Bool
  fun mix_set_position = Mix_SetPosition(channel : LibC::Int, angle : Int16, distance : UInt8) : Bool
  fun mix_set_distance = Mix_SetDistance(channel : LibC::Int, distance : UInt8) : Bool
  fun mix_set_reverse_stereo = Mix_SetReverseStereo(channel : LibC::Int, flip : LibC::Int) : Bool
  fun mix_reserve_channels = Mix_ReserveChannels(num : LibC::Int) : LibC::Int
  fun mix_group_channel = Mix_GroupChannel(which : LibC::Int, tag : LibC::Int) : Bool
  fun mix_group_channels = Mix_GroupChannels(from : LibC::Int, to : LibC::Int, tag : LibC::Int) : Bool
  ...
  fun mix_play_music = Mix_PlayMusic(music : MixMusic*, loops : LibC::Int) : Bool
  fun mix_fade_in_music = Mix_FadeInMusic(music : MixMusic*, loops : LibC::Int, ms : LibC::Int) : Bool
  fun mix_fade_in_music_pos = Mix_FadeInMusicPos(music : MixMusic*, loops : LibC::Int, ms : LibC::Int, position : LibC::Double) : Bool
  ...
  fun mix_fade_out_music = Mix_FadeOutMusic(ms : LibC::Int) : Bool
  ...
  fun mix_paused_music = Mix_PausedMusic() : Bool
  fun mix_mod_music_jump_to_order = Mix_ModMusicJumpToOrder(order : LibC::Int) : Bool
  fun mix_start_track = Mix_StartTrack(music : MixMusic*, track : LibC::Int) : Bool
  fun mix_get_num_tracks = Mix_GetNumTracks(music : MixMusic*) : LibC::Int
  fun mix_set_music_position = Mix_SetMusicPosition(position : LibC::Double) : Bool
  ...
  fun mix_playing_music = Mix_PlayingMusic() : Bool
  fun mix_set_sound_fonts = Mix_SetSoundFonts(paths : LibC::Char*) : Bool
  fun mix_get_sound_fonts = Mix_GetSoundFonts() : LibC::Char*
  fun mix_each_sound_font = Mix_EachSoundFont(function : MixEachSoundFontCallback, data : Void*) : Bool
  fun mix_set_timidity_cfg = Mix_SetTimidityCfg(path : LibC::Char*) : Bool
